...
#define REQUEST_HANDLER(name) void name(int fd)
...
main()
{
	...
	switch (tracker_child = fork()) {
		case 0:
			listen_fd = inetListen("5555", 5, NULL);
			handler = &handle_torrent_request;
			
			break;
		default:
			switch (web_child = fork()) {
				case 0:
					listen_fd = inetListen("8080", 5, NULL);
					handler = &handle_web_request;
			
					break;
				default:
					/* TODO: Make this process listen to stdin and
					 *       quit if asked to (stopping its children as
					 *       well) */
					waitpid(tracker_child, NULL, 0);
					waitpid(web_child, NULL, 0);
					break;
			}
	}
