...
send_peer_list(int fd, Torrent *torrent, Peer *asking_peer, int compact)
{
	...
	if (!compact) {
		...
		*(peer_list_write_pos++) = '\0';
		...
		int peer_list_size = actual_peer_list_count * PEER_COMPACT_BENCODE_LEN;
		...
		memcpy(full_tracker_response + full_tracker_response_size,
			   peer_list, peer_list_size);

		full_tracker_response_size + peer_list_size;
		...
		*(full_tracker_response + full_tracker_response_size++) = 'e';
		...
		response_write_pos += sprintf(response_write_pos,
									  "Content-Length: %d\r\n"	\
									  "\r\n",
									  full_tracker_response_size);

		memcpy(response_write_pos, full_tracker_response, full_tracker_response_size);
		response_write_pos += full_tracker_response_size;
		response_write_pos += sprintf(response_write_pos, "\r\n");
